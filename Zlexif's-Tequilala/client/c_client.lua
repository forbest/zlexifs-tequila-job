...
RegisterNetEvent('zlexif:Client:Notify')
AddEventHandler("zlexif:Client:Notify", function(msg,type)
    Notify(msg,type)
end)

AddEventHandler("zlexif:Client:Storage", function()
...
AddEventHandler("zlexif:Client:OpenTray01", function()
...
AddEventHandler("zlexif:Client:OpenTray02", function()
...
AddEventHandler("zlexif:Client:OpenTray03", function()
...
AddEventHandler("zlexif:Client:OpenTray04", function()
...
RegisterNetEvent("zlexif:Client:OpenShop", function(index)
...
AddEventHandler('zlexif:Client:Sit', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"sitchair"})
end)

-- || ===============> Invoice
 
RegisterNetEvent('zlexif:Client:Invoicing', function()
    if Config.JimPayments then
        ...
        if dialog then
            ...
            TriggerServerEvent("zlexif:Server:Billing", dialog.id, dialog.amount)
        end
    end
end)
...
function CraftNonAlcoholicMenu()
    ...
    for k, v in pairs(Config.NonAlcoholic) do
        ...
        item.text = text
        item.params = { event = 'zlexif:client:CraftNonAlcoholic', args = { type = k } }
...
function CraftNonAlcoholic(weapon)
    QBCore.Functions.Progressbar('CraftNonAlcoholic', Language.Progressbars.Make..Config.NonAlcoholic[weapon].label, 5000, false, false, {
        ...
        TriggerServerEvent('zlexif:server:CraftNonAlcoholic', Config.NonAlcoholic[weapon].hash)
        for k, v in pairs(Config.NonAlcoholic[weapon].materials) do
             TriggerServerEvent('zlexif:Server:RemoveItem', v.item, v.amount)
...
RegisterNetEvent('zlexif:client:CraftNonAlcoholic', function(data)
    QBCore.Functions.TriggerCallback("zlexif:server:Materials", function(hasMaterials)
...
function CraftAlcoholicMenu()
    ...
    for k, v in pairs(Config.Alcoholic) do
        ...
        item.text = text
        item.params = { event = 'zlexif:client:CraftAlcoholic', args = { type = k } }
        table.insert(columns, item)
    end
    exports['qb-menu']:openMenu(columns)
end
...
function CraftAlcoholic(weapon)
    QBCore.Functions.Progressbar('CraftAlcoholic', Language.Progressbars.Make..Config.Alcoholic[weapon].label, 6000, false, false, {
        disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = true,
    }, { animDict = "mini@repair", anim = "fixing_a_ped", }, {}, {}, function()
        Notify(Language.Notify.Make..Config.Alcoholic[weapon].label, 'success', 6000)
        TriggerServerEvent('zlexif:server:CraftAlcoholic', Config.Alcoholic[weapon].hash)
...
RegisterNetEvent('zlexif:client:CraftAlcoholic', function(data)
    QBCore.Functions.TriggerCallback("zlexif:server:Materials", function(hasMaterials)
        if (hasMaterials) then
...
RegisterNetEvent('zlexif:Client:GiveCoffee', function()
    ...
    Wait(5000)
    TriggerServerEvent('zlexif:Server:GiveCoffee')
end)
